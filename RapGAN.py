...
embedding_layer=Embedding(num_words+1,dim, input_length='None')
...
def context_model():
    g_input=Input(shape=(que_pad,))
    g_emb=embedding_layer(g_input)
    ...
    context_model=Model(g_input,sent_representation)
...
g_input=Input(shape=(que_pad,))
context=context_model(g_input)
action=Dense(num_words,activation="softmax")(context)
model = Model(g_input,action)
...
sent_representation2 = Lambda(lambda xin: K.sum(xin, axis=-2), output_shape=(dim,))(sent_representation2)

final_represent=concatenate([sent_representation,sent_representation2],axis=-1)



probabilities = Dense(2, activation='softmax')(final_represent)
...
def finding_max_reward(train_x,train_y,count):
   #make_trainable(g_model,True)
    ...
    return new_train,new_fake,reward
...
for x in range(2):
 #g_model.load_weights('partial_attention_half_attention.h5')   
    ...
    del X,Y
    print(result)
    #result=train_d(g_model,train_x,train_y)
    #print(result) 
...
f=open('2018/intermediate/context_attention.csv', 'w', newline='',encoding = 'utf-8')
